...
(@ARGV>=3)||die "usage $0 <reference file> <response file> <result file> [dict] [equal-pair file]\n";
...
while (<$FT>){
  ...
  for ($i=0; $i<$tn; $i++){
    ...
    if (!defined($rsent{$thead[$i]}) || ($ln[$i]=~/^\s*NULL\s*$/)  ){
      ...
      push(@tnx, $ln[$i]);
       ...
       $p_h+=$l_h; $p_d+=$l_d; $p_s=$l_s; $p_ii+=$l_ii;
       $a_h+=$l_h; $a_d+=$l_d; $a_s=$l_s; $a_ii+=$l_ii;

       print "BEST TEST SENT: $l_sent \n";
       print "BEST STATISTIC: hit=$l_h del=$l_d insert=$l_ii sub=$l_s wer=$least_wer\n\n";
  ...
  $p_wer=1-(($p_h+$p_d+$p_s)==0?0:($p_h-$p_ii)/($p_h+$p_d+$p_s));
  print "\n-------PERSON------\n";
  print "PERSON STATISTIC: hit=$p_h del=$p_d insert=$p_ii sub=$p_s  wer=$p_wer \n";
  print "-- ------PERSON------\n";
  ...
  $a_wer=1-(($a_h+$a_d+$a_s)==0?0:($a_h-$a_ii)/($a_h+$a_d+$a_s));
  print "\n\n-----------------------------TOTAL--------------------------------------\n";
...
sub read_equ_file
{
  ...
  if ($equfn ne ""){
       ...
       while (<FEQ>){
           ...
           #read in pair
           my @pa=split(/\s+/, $_);
           ((@pa >=2)&&(@pa<=3))||die "wrong format in pair file $equfn\n $_";
...
sub read_ref_file
{
  ...
  while (<FR>){
    ...
    (@t>=2)||die "error in the reference file $ref\n error line: $_\n";
    my $k=shift(@t);
    my $v=join(' ',@t);
    $v=clear_sent($v);
